...
public:
	...
	bool setLastName(const std::string& temp) {
		...
		if (temp.length() < 3) {
			Color::setRedTextColor();
			std::cout << "Error: Last name cannot be smaler than 2 characters.\n\n";
			Color::resetTextColor();
			return false;
		}
		...
		for (char ch : temp) {
			if (ch == ' ') {
				Color::setRedTextColor();
				std::cout << "Error: Last name must not contain spaces. Use an underscore (_) instead.\n\n";
				Color::resetTextColor();
				return false;
			}
			else if (isdigit(ch)) {
	...
	bool setMobilePhoneNumber(const std::string& temp) {
		...
		while(read>>tempDetails[0]>>tempDetails[1]>>tempDetails[2]>>tempDetails[3]>>tempDetails[4]>>tempDetails[5]){
			if(temp==tempDetails[5]){
				Color::setRedTextColor();
				std::cout<<"Error: Phone already exist\n\n";
				Color::resetTextColor();
				return false;
			}
		}
...
public:
	static void signUp() {
	    ...
	    userInput.inputConfirmedPassword();
	    ...
	    if (userInput.saveUserDataOnFile()) {
	        ...
	        std::cout << "Welcome, " << userDetails.getFirstName() << "!\n\n";
	        Color::resetTextColor();
	    }
		else{
	        Color::setRedTextColor();
	...
	bool login (const std::string& temp1,const std::string& temp2){
		...
		std::ifstream read("UserData.txt");
		std::string tempDetails[6]; // for user deatils
		// file fromat: username,password, firstname, last name, email, phone  
		while(read>>tempDetails[0]>>tempDetails[1]>>tempDetails[2]>>tempDetails[3]>>tempDetails[4]>>tempDetails[5]){
			if(temp2==tempDetails[1] && (temp1 ==tempDetails[0] || temp1 == tempDetails[4] || temp1==tempDetails[5])){
				std::cout<<"Creadentials matched.\n";
				return true;
			}	
		}
...
private:
    std::string title;
    std::string genre;
    std::string duration; // e.g. "2h 30m"
    std::string rating;
...
public:
    ...
    bool setGenre(const std::string& g);
    bool setDuration(const std::string& d);
    bool setRating(const std::string& r);
...
public:
    bool setDate(const std::string& d);
    bool setTime(const std::string& t);
    bool setHallName(const std::string& h);
    bool setAvailableSeats(int count);
		...
		while(true){
			...
			if (choice == "1") {
			    while (true) {
			        ...
			        std::cout << "Enter Username / Email / Phone (type 'close' to exit): ";
			        std::getline(std::cin, usernameOrContact);
			
			        if (usernameOrContact == "close") {
			            exit(0);
			        }
			        ...
			        Login login;
			        if (login.login(usernameOrContact, password)) {
			            Color::setGreenTextColor();
